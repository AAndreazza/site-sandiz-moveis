...
const SalaJantar = () => {
    ...
    return (
        <div className="bg-slate-50">
            <Seo title='Salas de Jantar - Sandiz Móveis'/>
            <WhatsappIcon />
            <NavBar />
                ...
                {dados.posts.edges.map(post => (
                    <div className='bg-gray-100 rounded-lg drop-shadow-xl mt-12 sm:mt-0 mx-8 sm:mx-0' key={post.node.nomeProduto}>
                        <Link to={getWhatsUrl(CTA_WHATSAPP_MENSAGENS.ctaButton)} target='blank'>
                            ...
                            <p className='mb-4 text-lg text-center text-gray-900'>{documentToReactComponents(JSON.parse(post.node.medidaProduto.raw), options)}</p>
