...
import NavBar from '../components/NavBarPages/NavBarColchoes'
...
const POSTS_QUERY = graphql`
    query {
        posts: allContentfulColchoes(filter: {visivel: {eq: true}}) {
        edges {
            node {
                nomeProduto
                visivel
                imagemProduto {
                    resize(height: 900, width: 1000) {
                    width
                    height
                    src
                    }
...
const Colchoes = () => {
    ...
    return (
        <div className="bg-slate-50">
            <Seo title='Colchões - Sandiz Móveis'/>
            <WhatsappIcon />
            <NavBar />
            <h2 class="pt-36 text-center text-4xl font-bold tracking-tight text-[#a72626] sm:text-6xl">COLCHÕES</h2>
            <div className='mt-4 border-t-2 border-[#a72626] sm:max-w-80 max-w-60 mx-auto pt-6'></div>
            <p class="text-center mx-6 sm:mx-auto text-lg sm:text-xl leading-8 text-[#9b2323] max-w-5xl">Renove seu quarto com os colchões da Sandiz Móveis. <br /> Escolha entre uma variedade de estilos disponíveis para <b>entrega imediata</b> ou <b>encomenda.</b></p>
