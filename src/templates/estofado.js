...
const Estofado = ({ data, ...props}) => {
    const conteudoRichJson = JSON.parse(data.contentfulEstofados.descricaoProduto.raw)

    return (
        <div class="bg-white px-6 mt-12 mb-12 sm:mt-16 sm:mb-16 lg:px-8">
            <h1>{data.contentfulEstofados.nomeProduto}</h1>
            <h3>Valor = {data.contentfulEstofados.valorProduto}</h3>
            <img src={data.contentfulEstofados.imagemProduto.resize.src} className='mt-10 mb-10 mx-auto sm:max-w-xl max-w-xs rounded-2xl' alt='imagem' />
...
export const pageQuery = graphql`
    query ($slug: String!) {
        contentfulEstofados(linkDirecionamento: {eq: $slug}, visivel: {eq: true}) {
        nomeProduto
        valorProduto
        descricaoProduto {
            raw
